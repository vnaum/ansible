...
Ansible prior to 2.1.0 had a limitation with regards to becoming an
...
* Upgrade to Ansible 2.1.0.  In Ansible 2.1.0, this behaviour was changed to
  use stdin to send the data to dd running on the remote machine rather than
  scp or sftp.  By doing this we are able to save the data as the sudo user
  instead of saving it as the ssh connection user.  If dd is not installed
  remotely, an error will be thrown.  Either install it or use one of the
  other workarounds:
